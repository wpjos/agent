...
本 skill 是一个**编排器（orchestrator）**，不直接实现贝叶斯优化或模型训练逻辑。它通过 JiuwenSwarm 框架的 Skill 调用机制，协调以下两个子 skill 完成闭环寻优：
...
当用户在 JiuwenSwarm 对话中触发本 skill 时，按以下步骤执行：
...
- 本 skill **不再支持** `python skills/hebo-forecasting-hpo/main.py --mode run` 离线全自助运行。如需脚本化执行，请直接使用 `bo-for-experiment` + `run_forecast_with_params.py`。
