...
实际执行由 JiuwenSwarm 框架根据 SKILL.md 中的编排说明，通过 Skill 工具调用以下两个子 skill 完成：

  - bo-for-experiment          : HEBO 参数空间管理与推荐
  - industrial-forecasting-skill : 工业时序预测训练与评估

使用方式：
  在 JiuwenSwarm 对话中触发本 skill，
  Agent 将自动执行：
...
如需脚本化执行，请直接使用：
  python skills/bo-for-experiment/main.py ...
...
注意：本 main.py 仅作说明，不执行实际 HPO。请在 JiuwenSwarm 对话中触发本 skill。
"""
...
def main() -> int:
    ...
    if args.workflow:
        ...
        print(WORKFLOW_TEXT)
        print('\n提示：本 skill 为 orchestrator，请在 JiuwenSwarm 对话中触发。')
